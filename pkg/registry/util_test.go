...
func initTestServerClient(serverPort int) (EPPTestServer, *Client, error) {
	eppTestServer, err := createEPPTestServer("127.0.0.1", serverPort)
	if err != nil {
		return EPPTestServer{}, nil, errors.Wrap(err, "Error when creating server for tests.")
	}

	eppTestClient, err := createEPPTestClient("test", "test123", "127.0.0.1", serverPort)
	if err != nil {
		return EPPTestServer{}, nil, errors.Wrap(err, "Error when creating client for tests.")
	}

	return eppTestServer, eppTestClient, nil
}
...
func createEPPTestClient(user, password, serverHost string, serverPort int) (*Client, error) {
	...
	client.SetSendWaitTime(time.Duration(10) * time.Microsecond)
...
var greeting = `<epp xmlns:obj="urn:ietf:params:xml:ns:obj-1.0" xmlns="urn:ietf:params:xml:ns:epp-1.0">
<greeting>
